  ...
  try {
    ...
    if (req.session.buildMode === 'complete' && req.session.compiledSpec) {
      ...
      enrichedNotes =
        ...
        `multi-file structure (index.html + css/style.css + js/app.js). ` +
        `Include a FIXED top navigation bar with AT LEAST 5 anchor links that smooth-scroll ` +
        `to clearly labelled in-page sections. Every section must have complete, realistic, ` +
        `domain-specific content — no placeholder text. NO multi-page routing ` +
        `(one index.html only; separate CSS and JS files are required).`;
